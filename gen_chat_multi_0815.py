...
from locallaw.SparkApiMark import sparkchat
...
def sparkchatbot(input):
    if input:
        messages.append({"role": "user", "content": input})
        chat = sparkchat(messages=messages)
        print(chat)
        return chat
...
def main():
    ...
    with gr.Blocks() as lawpoint:
        ...
        submit_btn.click(sparkchatbot, inputs=[input_prompt_txt], outputs=[outputs])
    ...
    multi_gr = gr.TabbedInterface(([lawpoint, lawcourt, local_knowledge]),
                                  ["法律考点", "模拟法庭", "本地知识库应用"])
    multi_gr.launch(auth=("admin", "pass1234"),server_name='0.0.0.0',server_port=8010)
